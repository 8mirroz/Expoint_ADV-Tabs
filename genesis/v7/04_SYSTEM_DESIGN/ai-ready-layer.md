...
- **Unified Knowledge Grounding**: Use the NotebookLM knowledge base (via Google Gemini 1.5 API + context caching) as the authoritative source for all technical and pricing advice.
...
- Support "Generative UI" actions (e.g., AI updates the 3D configurator based on user preferences).

**Non-Goals**:
- Training custom foundational models from scratch.
- Full autonomous voice-calling agents in Phase 1.
...
- **Backend Orchestration**: Next.js App Router API Routes.
- **LLM/Knowledge Base Integration**: **Google Gemini 1.5 Pro** with **Context Caching**. The knowledge base exported from NotebookLM is cached as a persistent system prompt, ensuring the assistant is an expert in Expoint's specific manufacturing nuances.
